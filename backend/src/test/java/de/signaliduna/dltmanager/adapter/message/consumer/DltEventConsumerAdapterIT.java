...
class DltEventConsumerAdapterIT extends AbstractSingletonContainerTest {
    ...
    void testIncomingDltEventWithValidPayload() {
        ...
        assertThat(dltEventRepo.findAllOrderedByLastAdminActionDesc()).hasSize(1);
    }
    ...
    void testIncomingDltEventWithNullData() {
        ...
        assertThat(dltEventRepo.findAllOrderedByLastAdminActionDesc()).isEmpty();
    }
    ...
    void testAdapterHandlesInvalidJsonDataDirectInvocation() {
        CloudEvent event = createTestCloudEvent("test-id-invalid", "not-json".getBytes(StandardCharsets.UTF_8));
        adapter.dltEventReceived().accept(event);
        assertThat(dltEventRepo.findAllOrderedByLastAdminActionDesc()).isEmpty();
    }
